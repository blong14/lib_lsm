...
pub const DatabaseSupervisor = struct {
    ...
    fn considerFreezingMemtable(self: *Self, data: u64) void {
        ...
        if (try db.transaction()) |txn| {
            defer txn.release();

            if ((txn.memtable.size() + data) >= db.Opts.sst_capacity) {
                self.queueAction(.{ .flush_memtable = .{ .id = txn.memtable.getId() } });
            }
        }
    }
    ...
    fn considerFlushingMemtable(self: *Self, id: []const u8) void {
        ...
        if (try db.transaction()) |txn| {
            ...
            var count: u64 = 0;
            for (txn.mem_cache.items.items) |memtable| {
                if (memtable.frozen()) count += 1;
            }
